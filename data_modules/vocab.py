...
class Vocab(object):
    def __init__(self, config, min_freq=1, special_token=['[PAD]', '[UNK]', '[SEP]', '[NONE]', '[EOS]'], max_size=None):
        """
        ...
        counter = Counter()
        self.config = config
        #### when generate the vocab, please use the following three keys
        ## When load the already generated vocabs, please use the below three keys: token, label, attribute
        # counter for tokens
        self.freqs = {'token': counter.copy(), 'label': counter.copy(), 'attribute': counter.copy()}
        # vocab to index
        self.v2i = {'token': dict(), 'label': dict(), 'attribute': dict()}
        # index to vocab
        self.i2v = {'token': dict(), 'label': dict(), 'attribute': dict()}
        ...
        attribute_dir = os.path.join(self.config.vocabulary.dir, self.config.vocabulary.attribute_dict)
        vocab_dir = {'token': token_dir, 'label': label_dir, 'attribute':attribute_dir}
        if os.path.isfile(label_dir) and os.path.isfile(token_dir):
            ...
            with open(attribute_dir, 'r') as f_in:
                for i, line in enumerate(f_in):
                    data = line.rstrip().split('\t')
                    assert len(data) == 2
                    self.v2i['attribute'][data[0]] = i
                    self.i2v['attribute'][i] = data[0]
        ...
        self.padding_index = self.v2i['token']['[PAD]']
        self.oov_index = self.v2i['token']['[UNK]']
        self.sep_index = self.v2i['token']['[SEP]']
        self.none_index = self.v2i['token']['[NONE]']
        self.eos_index = self.v2i['token']['[EOS]']
