...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        batch_token = []
        batch_label = []
        batch_label_nestedlist = []
        batch_doc_len = []
        batch_generate_y = []
        batch_generate_y_lengths = []
        batch_gating_label = []
        for sample in batch:
            batch_token.append(sample['token'])
            batch_label.append(sample['label'])
            batch_doc_len.append(sample['token_len'])
            batch_label_nestedlist.append(sample['label_nestedlist'])
            batch_generate_y.append(sample['generate_y'])
            batch_generate_y_lengths.append(sample['y_lengths'])
            batch_gating_label.append(sample['gating_label'])

        batch_token = torch.tensor(batch_token)
        batch_multi_hot_label = self._multi_hot(batch_label)
        batch_doc_len = torch.tensor(batch_doc_len)
        #batch_doc_len = torch.FloatTensor(batch_doc_len)
        batch_generate_y = torch.tensor(batch_generate_y)
        batch_generate_y_lengths = torch.tensor(batch_generate_y_lengths)
        batch_gating_label = torch.tensor(batch_gating_label)
        return {
            'token': batch_token,
            'label': batch_multi_hot_label,
            'token_len': batch_doc_len,
            'label_list': batch_label,
            'label_nestedlist': batch_label_nestedlist,
            'generate_y': batch_generate_y,
            'y_lengths': batch_generate_y_lengths,
            'gating_label': batch_gating_label
        }
